...
def _rdm_job(Xseed, distance, seed_id, total, verbose=0):
    ...
    if verbose:
        print("sphere {}/{} has {} values".format(seed_id, total, nvoxels))
...
def normalized_rdm(rdm, norm, vmin, vmax):
    ...
    n = rdm.shape[0]
    i = np.triu_indices(
        n, k=1) if norm == "upper" else np.tril_indices(n, k=-1)
    rdm[i] = norm_rdm[i]
    return rdm
...
#     print(vect)
#     plot_rdm(vect, norm="upper", labels=labels, vmin=0,
#              vmax=20, include_diag=True, title="Test RDM")
#     plt.show()
